...
fun MultiEventHandler<*>.getEventHandlers(eventClass: KClass<out Event>): List<KFunction<*>> {
    ...
    return this::class.functions.filter {
        ...
        check(
            ...
            "MultiEventHandler error in ${eventClass.qualifiedName}: " + if (allowsData)
...
internal fun <DATA> MultiEventHandler<DATA>.getEventDataGetters(eventClass: KClass<out Event>): List<KFunction<DATA>> =
    this::class.functions.filter {
        ...
        check(it.parameters.size == 2) { "MultiEventHandler error in ${eventClass.qualifiedName}: Event data getter $it must have no parameters." }
...
internal fun MultiEventHandler<*>.getEventFilters(eventClass: KClass<out Event>): List<KFunction<Boolean>> =
    this::class.functions.filter {
        ...
        check(it.parameters.size == 2) { "MultiEventHandler error in ${eventClass.qualifiedName}: Event filter $it must have no parameters." }
...
fun <DATA> MultiEventHandler<DATA>.registerAllEventHandlers(eventNode: EventNode<Event>) {
    getEventHandlers(Event::class).forEach { eventHandler ->
        ...
        var dataGetter = getEventDataGetters(eventClass).reversed().firstOrNull()
        if (eventHandler.parameters.size == 3) {
            dataGetter =
                requireNotNull(dataGetter) { "MultiEventHandler error in ${eventClass.qualifiedName}: No data getter for event ${eventClass.qualifiedName}." }
        }
